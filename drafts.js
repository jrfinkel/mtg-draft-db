...
function firstLineup (body, response) {
	...
	syncQuery(playerQuery(team2Players), function(players2) {
		...
		'<input type=hidden name="team1" value="'+ utils.htmlEncode(JSON.stringify(team1Players)) +'">' +
		'<input type=hidden name="team2" value="'+ utils.htmlEncode(JSON.stringify(team2Players)) +'">' +
...
function secondLineup (body, response) {
    var team1Players = JSON.parse(utils.htmlDecode(body['team1']));
    var team2Players = JSON.parse(utils.htmlDecode(body['team2']));
